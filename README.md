...
- while `r_value` is not zero:
  - if the bit at the current position is 1, advance `r_value` $2^{position}$ steps
  - add 1 to the bit position

At the end of this process, `r_value` will be exactly $0$.  This works because each time `r_value` is advanced $2^{position}$ steps, the bit at that position is toggled, and the bits below that position are unchanged.  (The advancement is a multiple of the cycle length of the lower bits; the upper bits will change however.)  The total advancements equals the distance to $r_0$, which you can then subtract from $2^{32}$ to find the original `r_count`.
